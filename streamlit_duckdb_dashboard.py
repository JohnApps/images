...
class DuckDBExplorer:
    ...
    def _init_connection(_self):
        ...
        return duckdb.connect(_self.db_path, read_only=True)
    ...
    @st.cache_data(ttl=300)
    def get_table_schema(_self, table_name):
    ...
    @st.cache_data(ttl=300)
    def get_table_stats(_self, table_name):
        """Get basic statistics for a table"""
        row_count = _self.conn.execute(f"SELECT COUNT(*) FROM {table_name}").fetchone()[0]
        ...
        """
        # DuckDB doesn't have pg_size functions, so we'll skip size
        
        return {
            'row_count': row_count,
            'column_count': len(_self.get_table_schema(table_name))
        }
    
    @st.cache_data(ttl=60)
    def query_table(_self, table_name, limit=1000, offset=0, where_clause=None):
    ...
    @st.cache_data(ttl=300)
    def get_column_stats(_self, table_name, column_name, data_type):
        """Get statistics for a specific column"""
        stats = {}
        
        try:
            if 'INT' in data_type.upper() or 'DOUBLE' in data_type.upper() or 'DECIMAL' in data_type.upper():
                query = f"""
                SELECT 
...
def create_scatter_matrix(df, columns):
    ...
    fig = px.scatter_matrix(df, dimensions=columns[:5], 
